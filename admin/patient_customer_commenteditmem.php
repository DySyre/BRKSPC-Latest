...
$sql3 = "SELECT * FROM `appointment_tbl` WHERE appointment_payment_id = '$idBill'";
...
<div class="row">
  <form id="paymentForm">
   ...
   <div class="col-md-12"> 
    ...
    <textarea id="payments"><?php echo $row3['appointment_coment']; ?></textarea>
  </div>
  </div>
...
$('#paymentForm').on('click', '.btnProceed', function(event) {
 ...
 var payment = document.getElementById("payments").value;
  ...
  $.ajax({
    url: "patient_customer_commenteditmem_insert.php",
    method: "POST",
    data: {
      idd: idd,
      payment: payment,
