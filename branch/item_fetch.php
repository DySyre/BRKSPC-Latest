...
$output= array();
$sql = "SELECT * FROM `item_tbl` JOIN item_category_tbl ON item_tbl.item_categoryidfk = item_category_tbl.item_category_id JOIN branch_tbl on item_category_tbl.item_category_branch = branch_tbl.branch_id where item_category_branch ='$branchId'";
...
if(isset($_POST['search']['value']))
{
	...
	$sql .= " AND  (item_code like '%".$search_value."%'";
...
while($row = mysqli_fetch_assoc($query))
{
		...
		$sub_array[] = '<a href="javascript:void();" data-id="'.$row['item_id'].'"  class="btn btn-primary btn-sm addstock" style="background-color: ;border: none;">Add Stock</a> <a href="javascript:void();" data-id="'.$row['item_id'].'"  class="btn btn-sm editbtnClassroom btn-light" style="background-color: transparent;border: none;"><i class="bi bi-pen-fill" style="color: #FFC107"></i></a>  <a href="javascript:void();" data-id="'.$row['item_id'].'"  class="btn btn-danger btn-sm deleteBtnClassroom" style="background-color: transparent;border: none;"><i class="bi bi-trash3-fill" style="color: red"></i></a>';
		$data[] = $sub_array;
